...
- **Carte, couches live** : les coordonnées des centres (données publiques) sont envoyées à Open-Meteo depuis le serveur ATLAS, jamais depuis l'appareil de l'agent ; Météo-France, Hub'Eau et l'IGN ne reçoivent aucune donnée d'agent. Les choix de couches restent sur l'appareil.
- **Hors ligne** : la file d'envoi différé (réactions, messages texte) et les 20 dernières publications sont gardées sur l'appareil seulement, effacées à l'envoi ou au remplacement. Le journal d'audit (`audit_log`) trace les actions du studio avec leur auteur ; il est consultable par les administrateurs (tout) et les éditeurs (leurs actions), exportable en CSV. `/api/health` n'expose aucune donnée personnelle.
