...
- **Vidéo** : **compression sur l'appareil** (`lib/media/compress.ts`, WebCodecs + `mp4-muxer`) quand le navigateur le permet (Safari 17.4+, Chrome 94+) : tout format lisible (HEVC d'iPhone compris) ressort en MP4 H.264 1080p 30 i/s (4,2 Mbit/s) + AAC 96 kbit/s, fast start. À défaut, chemin historique : lecture minimale des boîtes MP4 (`lib/media/mp4.ts`), `avc1` exigé, MOV renommé. Côté serveur, `faststart` replace `moov` en tête si besoin. Limite `NEXT_PUBLIC_UPLOAD_MAX_MB` (50 Mo par défaut, plafond de Supabase Storage gratuit) appliquée au presign et au message 413.
- **Images** : réduites sur l'appareil avant envoi (2400 px de côté, JPEG 86 %) ; variantes WebP 400 / 800 / 1200 / 2400 px côté serveur, l'original est effacé après traitement ; `srcset` laisse le téléphone choisir la taille.
...
- **Mobile-first** : BottomNav (Fil, Agenda, Favoris, Profil, + Studio pour éditeurs et administrateurs), TopBar en verre avec le logo. Les autres écrans gardent un grand titre (34 px) qui se replie dans la barre au scroll ; le fil n'en a pas (stories puis publications directement). Studio : sidebar desktop, utilisable sur tablette.
